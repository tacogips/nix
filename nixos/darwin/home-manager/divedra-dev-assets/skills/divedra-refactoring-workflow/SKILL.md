...
description: Use when planning, running, monitoring, or troubleshooting the user-scope divedra divide-and-conquer refactoring workflow. Applies to `~/.divedra/workflows/refactoring-divide-and-conquer`, duplicate-scavenge refactoring, concurrent slice review via `refactoring-slice-review`, generated refactoring implementation plans, implementation/review loop handling, blocked verification, dependency-owned agent mocks, and final plan archival.
---
...
```bash
divedra workflow run refactoring-divide-and-conquer --scope user \
...
```bash
divedra workflow validate refactoring-divide-and-conquer --scope user --output json
divedra workflow validate refactoring-slice-review --scope user --output json
```
