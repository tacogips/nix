---
name: divedra-codex-refactoring-workflow
description: Use when planning, running, monitoring, or troubleshooting the user-scope Codex-agent divide-and-conquer refactoring workflows. Applies to `~/.divedra/workflows/codex-refactoring-divide-and-conquer` and `codex-refactoring-slice-review` (executionBackend codex-agent), duplicate-scavenge refactoring, generated refactoring implementation plans, implementation/review loop handling, blocked verification, dependency-owned agent mocks, and final plan archival. For Cursor-agent workflows, use the future cursor-* workflow and skill names instead.
---

# Divedra Codex Refactoring Workflow

Use this skill when the user asks to refactor through the Codex-agent divide-and-conquer workflow, asks whether the refactoring workflow is progressing, or asks to create/continue a maintainability refactor using Codex workflow review loops.

## Codex Workflow Bundle

- Parent workflow: `~/.divedra/workflows/codex-refactoring-divide-and-conquer`
- Child workflow: `~/.divedra/workflows/codex-refactoring-slice-review`
- Agent backend: `codex-agent` on every workflow step
...
```bash
divedra workflow run codex-refactoring-divide-and-conquer --scope user \
...
```bash
divedra workflow validate codex-refactoring-divide-and-conquer --scope user --output json
divedra workflow validate codex-refactoring-slice-review --scope user --output json
```
