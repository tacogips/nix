...
get_highest_cpu_temp() {
    ...
    if [ -z "$cpu_temp" ] || [ "$cpu_temp" == "null" ]; then
        ...
        cpu_temp=$(sensors 2>/dev/null | grep -E 'Core|Tctl|Package id' | awk '{print $2}' | tr -d "+u00b0C" | sort -rn | head -1)
    fi
    ...
    if [ -n "$cpu_temp" ] && [ "$cpu_temp" != "null" ]; then
        ...
        echo "{ \"text\": \"$cpu_tempu00b0C\", \"tooltip\": \"CPU Max Temperature: $cpu_tempu00b0C\" }"
    else
