...
The shared fish functions also provide iterative agent loops on both Linux and Darwin: `codex-loop`, `codex-loop-review-today`, `cursor-loop`, and `cursor-loop-review-today`. These helpers append the same architecture and git-diff review suffix on each iteration, and the fixed `*-review-today` variants reuse the same review prompt as `co-review-today`.
The shared agent configuration now installs the `secure-github-action` user skill for both Codex and Claude on Linux and Darwin. That keeps `~/.codex/skills/secure-github-action/` and `~/.claude/skills/secure-github-action/` in sync from the same repository-managed source, so workflow hardening guidance is available in both CLIs.
...
The tmux footer now renders on each pane border. It shows the current repository name for Git worktrees, appends the current branch, and falls back to the current directory name outside Git repositories.
