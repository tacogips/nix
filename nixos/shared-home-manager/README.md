...
On Darwin, Home Manager copies the development workflows from the local `~/gits/tacogips/divedra` checkout into the user-scope catalog at `~/.divedra/workflows`, and copies the related Divedra workflow operation skills from that checkout into `~/.agents/skills`. It also copies Divedra's `git-new-branch` skill for branch creation through managed worktrees, plus `ts-coding-standards` and `ts-review` so user-scope reviews can apply the non-gitleaks security checklist for machine-local paths, credential material, and private URLs.
