...
The shared tmux workflow now exposes predefined layouts directly inside tmux. `Alt-t` still opens a plain new window rooted at the current pane's directory. `Alt-i` opens a new window and immediately shows a layout menu, while `prefix + I` shows the same menu for the current window and rebuilds the window from the active pane before applying the selected layout. The `ide-3pane` layout starts Yazi on the left, opens files in Helix in the center pane, and retargets directory selections to the right pane's current working directory.
