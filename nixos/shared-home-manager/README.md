...
The shared agent configuration now installs the `secure-github-action` and `envrc-generate` user skills for both Codex and Claude on Linux and Darwin. That keeps `~/.codex/skills/` and `~/.claude/skills/` in sync from repository-managed sources, so workflow hardening guidance and standard `.envrc` generation are available in both CLIs.
