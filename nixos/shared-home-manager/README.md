...
Swift editor integration enables the `sourcekit` LSP in Neovim. On Darwin, Neovim launches it through `/usr/bin/xcrun sourcekit-lsp` so iOS and macOS app projects use the selected Xcode toolchain and SDK; on non-Darwin systems, Home Manager installs the Nix-provided `sourcekit-lsp`.
