...
On Darwin, Home Manager installs the checked-in Divedra workflow assets from `nixos/darwin/home-manager/divedra-dev-assets/` into the user-scope catalog at `~/.divedra/workflows`, and installs the related checked-in Divedra workflow operation skills into `~/.agents/skills`. This does not require a local `divedra` source checkout on the target machine. It also installs Divedra's `git-new-branch` skill for branch creation through managed worktrees, plus `ts-coding-standards` and `ts-review` so user-scope reviews can apply the non-gitleaks security checklist for machine-local paths, credential material, and private URLs.
