...
Swift editor integration enables the `sourcekit` LSP in Neovim. On Darwin, Home Manager installs a `sourcekit-lsp` wrapper that launches `/usr/bin/xcrun sourcekit-lsp` with `DEVELOPER_DIR` defaulting to `/Applications/Xcode.app/Contents/Developer`, so Swift projects use the selected Xcode toolchain rather than a Nix Swift toolchain. On non-Darwin systems, Home Manager installs the Nix-provided `sourcekit-lsp`.
